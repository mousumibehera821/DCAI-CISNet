...
def train_init():
    ...
    if not torch.cuda.is_available():
        print("⚠️ No GPU found, switching to CPU")
...
def train(epoch):
    ...
    for batch in tqdm(training_data_loader):
        ...
        gt_rgb = im2
        output_hvi = model.DCAI(output_rgb)
        gt_hvi = model.DCAI(gt_rgb)
        ...
        loss = loss_rgb + opt.DCAI_weight * loss_hvi
        iter += 1
...
if __name__ == '__main__':  
    ...
    with open(f"./results/training/metrics{now}.md", "w") as f:
        ...
        f.write(f"DCAI_weight: {opt.DCAI_weight}\n")  
